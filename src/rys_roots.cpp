...
#ifdef HAVE_QUADMATH_H
static int R_qroot(int nroots, double x, double *roots, double *weights);
#else
static int R_lroot(int nroots, double x, double *roots, double *weights);
#endif
...
void CINTerfc_rys_roots(int nroots, double x, double lower, double *u, double *w)
{
        int error;
        switch (nroots) {
...
static int R_lroot(int nroots, double x,
                   double roots[], double weights[])
{
        int i, k, j, m, order;
        int nroots1 = nroots + 1;
        ...
        long double fmt_ints[MXROOTS*2+1];
        long double *a;
        ...
        if (nroots == 1) {
                ...
                dum = SQRTL(cs[2*MXROOTS1+1] * cs[2*MXROOTS1+1] - 4 * cs[2*MXROOTS1+0] * cs[2*MXROOTS1+2]);
                rt[0] = .5 * (-cs[2*MXROOTS1+1] - dum) / cs[2*MXROOTS1+2];
                rt[1] = .5 * (-cs[2*MXROOTS1+1] + dum) / cs[2*MXROOTS1+2];
                for (i = 2; i < nroots1; i++) {
                        rt[i] = 1;
...
static int R_qroot(int nroots, double x, double *roots, double *weights)
{
        ...
        int error = R_qsmit(cs, fmt_ints, nroots1);
#ifndef KEEP_GOING
        if (error) {
                exit(error);
        }
